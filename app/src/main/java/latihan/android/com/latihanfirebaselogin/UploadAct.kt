...
import kotlinx.android.synthetic.main.upload_foto.*
...
class UploadAct: AppCompatActivity() {
            ...
            when{
                (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)->{
                    if (ContextCompat.checkSelfPermission(
                            this@UploadAct, Manifest.permission.READ_EXTERNAL_STORAGE)!= PackageManager.PERMISSION_GRANTED){
    ...
    private fun UploadFile() {
        ...
        var value =  0.0
        var storage = data.getReference().child("mypic.jpg")
        storage.putFile(filePath).addOnProgressListener {
            taskSnapshot ->
